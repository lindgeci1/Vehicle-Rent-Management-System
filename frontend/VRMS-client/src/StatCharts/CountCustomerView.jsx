...
import { decodeToken } from "../../decodeToken"; // ✅ Adjust if needed
import {
  CalendarDaysIcon,
  CreditCardIcon,
  ReceiptRefundIcon,
...
function CountCustomerView() {
  ...
  return (
    <Card className="p-6 shadow-sm rounded-md border border-blue-gray-100">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div className="text-center" key={key}>
            {iconMap[key]}
            <Typography className="text-sm text-blue-gray-600 mb-1">
              {label}
            </Typography>
            <Typography variant="h5" className="font-bold" style={{ color: "#64748B" }}>
              {value}
            </Typography>
          </div>
        ))}
